...
from app.forms import SignUpForm, LogInForm, ProfileForm, EditProfileForm
from app.models import MonUser, Profile
...
@ratelimit(key='ip', rate='10/m', method='GET', block=True)
@login_required
def home(request: Any, user_id: int) -> HttpResponse:
    return render(request, "app/home.html")


@ratelimit(key='ip', rate='10/m', method='GET', block=True)
@login_required
def create_profile(request: Any, user_id: int) -> HttpResponse:
    user = MonUser.objects.get(id=user_id)
...
@ratelimit(key='ip', rate='10/m', method='GET', block=True)
@login_required
def edit_profile(request: Any, user_id: int) -> HttpResponse:
    user = MonUser.objects.get(id=user_id)
    ...
    if request.method == "POST" and form.is_valid():
        ...
        profile.save()
        return redirect("profile", user_id=user_id)
    return render(request, "app/edit_profile.html", {"user_id": user_id, "form": form})


@ratelimit(key='ip', rate='10/m', method='GET', block=True)
@login_required
def add_server(request: Any, user_id: int) -> HttpResponse:
