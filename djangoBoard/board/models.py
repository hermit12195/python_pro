...
class User(AbstractUser):
    ...
    def ads_list(self):
        ...
        """
        try:
            res: list = [ad.title for ad in self.ads.all()]
...
class Category(models.Model):
    ...
    def active_ads(self):
        ...
        """
        try:
            res: QuerySet = self.ads.filter(is_active=True).count()
...
class Ad(models.Model):
    ...
    def comments_number(self):
        ...
        """
        try:
            res: QuerySet = self.comments.count()
...
def remove_old_ads():
    ...
    """
    try:
        ads: QuerySet = Ad.objects.filter(created_at__lt=timezone.now() - timedelta(days=30))
        inactive_ads: QuerySet = ads.update(is_active=False)
